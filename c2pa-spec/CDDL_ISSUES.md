...
These CDDL files in `schemas/cddl/` fail to parse with the `cddl` crate
...
## validation-results.cddl (spec ≤ 2.3)

```
semver-string /= tstr .regexp "^(0|[1-9]\d*)\.(0|[1-9]\d*)..."
```

The SemVer regex uses single-backslash escapes (`\d`, `\.`, `\+`) inside
a CDDL string literal. Those aren't in the set of escapes that RFC 8610
permits; the parser rejects them. Spec 2.4 fixed this by doubling the
backslashes (`\\d`, `\\.`, `\\+`), so this exclusion is only needed
while we're ingesting any bundle ≤ 2.3. Safe to remove once 2.4 is the
floor.
