...
/// when building the master schema. See `c2pa-spec/CDDL_ISSUES.md`.
const EXCLUDED_CDDLS: &[&str] = &[
    "ai-disclosure.cddl",
    "jsonld.cddl",
    "regions-of-interest.cddl",
    "time-stamp.cddl",
    "validation-results.cddl",
    "c2pa.cddl",
];
...
fn main() {
    ...
    let master = build_master_cddl(&cddl_dir);
...
/// twice.
fn build_master_cddl(cddl_dir: &Path) -> String {
    ...
    out.push_str(&EXCLUDED_CDDLS[..EXCLUDED_CDDLS.len() - 1].join(", "));
    out.push_str("\n\n");
